...
def add_noise(data, snr_db):
    # This function will fail if array's gain is not uniform
    ## Obtain the noise power 
    sig_pwr = np.abs(data[0,:]) ** 2
    
    # Ensure non-zero values for the logarithm
    epsilon = 1e-10
    ...
    # noise = np.sqrt(noise_avg / 2) * np.random.randn(data.shape[0], data.shape[1]) + 1j * np.sqrt(noise_avg / 2) * np.random.randn(data.shape[0], data.shape[1])
    signal_n = data + noise
...
def MDL_test(data):
    ...
    for d in range(num_elem):
        ...
        # mdl_metric[d,0] = Ld + (d*(2*num_elem-d+1))*np.log(num_samples)/2
...
def doa_MUSIC_ULA(R_xx, delta, num_signals, resolution):
    ...
    for i in range(resolution):
        ...
        # p_theta = np.abs(1 / (a_i.conj().T @ Vn @ Vn.conj().T @ a_i))
